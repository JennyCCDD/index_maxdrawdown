...
for i in range(len(datas.index)-1):
    maxdd,_,_  = MaxDrawdown(datas.iloc[i:i + win], colname='close', period='all')
    maxl,_,_  = MaxLoss(datas.iloc[i:i + win],colname = 'close', period='all')
...
maxddbound = 0.4
datas4['maxdd_backtime_bound=%s'%maxddbound]=[0]*len(datas4)
datas4['flag'] = [0]*len(datas4)
datas4.loc[datas4['maxdd']>maxddbound]['flag'] = 1
dates4 = datas4.loc[datas4['maxdd']>maxddbound,'number'].tolist()
...
for date_i, index_i in enumerate(dates4):
    ...
    if len(dates_back) > 1:
        ...
        datas4.loc[datas4['number'] ==  max(dates_back),'maxdd_backtime_bound=%s' % maxddbound] \
            = backtime

datas4.to_csv('./result/datas4.csv',encoding='utf8')
